...
fn write_stored_block(output: &mut Vec<u8>, bytes: &[u8], final_block: bool) -> Result<()> {
    let len = u16::try_from(bytes.len())
        .map_err(|_| MediaError::invalid_argument("DEFLATE stored block exceeds 65535 bytes"))?;
...
mod tests {
    ...
    fn standard_fixed_huffman_zlib_vector_decodes() {
        let stream = [
            0x78, 0x9C, 0xCB, 0x48, 0xCD, 0xC9, 0xC9, 0x57, 0xC8, 0x40, 0x27, 0x15, 0x01, 0x70,
            0xD5, 0x08, 0xD2,
        ];
        assert_eq!(
    ...
    fn dynamic_huffman_zlib_wrapper_matches_verified_raw_payload() {
        let mut raw = vec![
            ...
            0x26, 0x00, 0x09, 0xC9, 0x97, 0x64, 0x9D, 0xD5, 0x76, 0xE6, 0x46,
        ];
