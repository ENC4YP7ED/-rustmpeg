...
fn decode_huff(reader: &mut BitReader<'_>, table: &Huffman) -> Result<u8> {
    ...
    for len in 1..=16 {
        ...
        if code >= first && code < first + count {
            return table
                .symbols
                .get(index + usize::try_from(code - first).unwrap())
                .copied()
...
fn entropy_unstuff(data: &[u8]) -> Result<Vec<u8>> {
    ...
    while i < data.len() {
        ...
        let next = *data
            .get(i + 1)
            .ok_or_else(|| MediaError::eof("truncated JPEG entropy byte stuffing"))?;
        match next {
            0x00 => {
                out.push(0xff);
                i += 2;
            ...
            _ => {
                return Err(MediaError::invalid_data(
                    "unexpected marker inside JPEG entropy payload",
                ));
            }
        }
    }
...
pub fn decode_jpeg(bytes: &[u8]) -> Result<VideoFrame> {
    if bytes.len() < 4 || bytes[..2] != [0xff, 0xd8] {
        return Err(MediaError::invalid_data("missing JPEG SOI"));
    }
    let mut pos = 2usize;
    let mut width = 0u16;
    let mut height = 0u16;
    ...
    while pos + 1 < bytes.len() {
        if bytes[pos] != 0xff {
            return Err(MediaError::invalid_data("expected JPEG marker"));
        }
        ...
        if pos + 2 > bytes.len() {
            return Err(MediaError::eof("truncated JPEG segment length"));
        }
        let len = usize::from(u16::from_be_bytes([bytes[pos], bytes[pos + 1]]));
        if len < 2 || pos + len > bytes.len() {
            return Err(MediaError::eof("truncated JPEG segment"));
        }
        ...
        match marker {
            0xdb => {
                ...
                while p < seg.len() {
                    ...
                    let id = usize::from(spec & 15);
                    if id > 3 || precision > 1 {
                        return Err(MediaError::invalid_data("invalid JPEG DQT"));
                    }
            ...
            0xc4 => {
                ...
                while p < seg.len() {
                    ...
                    let id = usize::from(spec & 15);
                    if class > 1 || id > 3 {
                        return Err(MediaError::invalid_data("invalid JPEG DHT"));
                    }
                    let counts_slice = seg
                        .get(p..p + 16)
            ...
            0xc0 => {
                if seg.len() < 6 || seg[0] != 8 {
                    return Err(MediaError::unsupported(
                        "only 8-bit baseline JPEG is decoded",
                    ));
                }
                ...
                if n != 1 && n != 3 {
                    return Err(MediaError::unsupported(
                        "baseline JPEG supports 1 or 3 components",
                    ));
                }
                if seg.len() != 6 + n * 3 {
                    return Err(MediaError::invalid_data("JPEG SOF0 size mismatch"));
                }
                components.clear();
                for c in seg[6..].chunks_exact(3) {
                    ...
                    if h == 0 || v == 0 || h > 4 || v > 4 || c[2] > 3 {
                        return Err(MediaError::invalid_data("invalid JPEG component"));
                    }
            ...
            0xc2 => {
                return Err(MediaError::unsupported(
                    "progressive JPEG decode is not implemented yet",
                ));
            }
            0xda => {
                if components.is_empty() {
                    return Err(MediaError::invalid_data("JPEG SOS before SOF0"));
                }
                let n = usize::from(
                    *seg.first()
                        .ok_or_else(|| MediaError::eof("empty JPEG SOS"))?,
                );
                if seg.len() != 1 + n * 2 + 3 || n != components.len() {
                    return Err(MediaError::unsupported(
                        "only single-scan interleaved baseline JPEG is decoded",
                    ));
                }
                for i in 0..n {
                    let id = seg[1 + i * 2];
                    let selectors = seg[2 + i * 2];
                    let c = components
                        .iter_mut()
                        .find(|c| c.id == id)
                        .ok_or_else(|| MediaError::invalid_data("unknown JPEG SOS component"))?;
                    c.dc_table = selectors >> 4;
                    c.ac_table = selectors & 15;
                }
                if seg[1 + n * 2] != 0 || seg[2 + n * 2] != 63 || seg[3 + n * 2] != 0 {
                    return Err(MediaError::unsupported(
                        "non-sequential JPEG scan parameters",
                    ));
                }
                let start = pos;
                let mut end = pos;
                while end + 1 < bytes.len() {
                    if bytes[end] == 0xff {
                        ...
                        if code == 0x00 || code == 0xff || (0xd0..=0xd7).contains(&code) {
                            end += 2;
                            continue;
                        }
                        break;
    ...
    if width == 0 || height == 0 || components.is_empty() {
        return Err(MediaError::invalid_data("JPEG missing baseline frame"));
    }
    ...
    let mcus_x = (usize::from(width) + mcu_w - 1) / mcu_w;
    let mcus_y = (usize::from(height) + mcu_h - 1) / mcu_h;
    let mut planes: Vec<Vec<u8>> = components
        .iter()
        .map(|c| {
            let pw = mcus_x * usize::from(c.h) * 8;
            let ph = mcus_y * usize::from(c.v) * 8;
            vec![0; pw * ph]
        })
        .collect();
    let plane_widths: Vec<usize> = components
        .iter()
        .map(|c| mcus_x * usize::from(c.h) * 8)
        .collect();
    ...
    for my in 0..mcus_y {
        for mx in 0..mcus_x {
            for ci in 0..components.len() {
                ...
                let dct = dc[usize::from(c.dc_table)]
                    .as_ref()
                    .ok_or_else(|| MediaError::invalid_data("missing JPEG DC table"))?;
                let act = ac[usize::from(c.ac_table)]
                    .as_ref()
                    .ok_or_else(|| MediaError::invalid_data("missing JPEG AC table"))?;
                for by in 0..usize::from(c.v) {
                    for bx in 0..usize::from(c.h) {
                        ...
                        while k < 64 {
                            ...
                            coeff[ZIGZAG[k]] =
                                receive_extend(&mut br, s)? * i32::from(qt[ZIGZAG[k]]);
                            k += 1;
                        }
                        let block = idct(&coeff);
                        let pw = plane_widths[ci];
                        let ox = (mx * usize::from(c.h) + bx) * 8;
                        let oy = (my * usize::from(c.v) + by) * 8;
                        for y in 0..8 {
                            let dst = (oy + y) * pw + ox;
                            planes[ci][dst..dst + 8].copy_from_slice(&block[y * 8..y * 8 + 8]);
                        }
                    }
                }
            }
    ...
    if components.len() == 1 {
        ...
        let mut out = vec![0u8; usize::from(width) * usize::from(height)];
        for y in 0..usize::from(height) {
            out[y * usize::from(width)..(y + 1) * usize::from(width)]
                .copy_from_slice(&planes[0][y * pw..y * pw + usize::from(width)]);
        }
    ...
    for y in 0..usize::from(height) {
        for x in 0..usize::from(width) {
            ...
            for ci in 0..3 {
                let c = components[ci];
                let sx = x * usize::from(c.h) / usize::from(max_h);
                let sy = y * usize::from(c.v) / usize::from(max_v);
