...
fn temp_dir(label: &str) -> PathBuf {
    ...
    let path = std::env::temp_dir().join(format!("rustmpeg-jpeg-{label}-{}-{nonce}", std::process::id()));
...
fn ffmpeg() -> Command { Command::new(env!("CARGO_BIN_EXE_ffmpeg")) }
...
fn assert_near(actual: &[u8], expected: &[u8], tolerance: u8) {
    ...
    for (i, (&a, &e)) in actual.iter().zip(expected).enumerate() {
        assert!(a.abs_diff(e) <= tolerance, "sample {i}: got {a}, expected {e}, tolerance {tolerance}");
    }
}
...
fn rgb_ppm_to_jpeg_and_back_is_visually_bounded() {
    ...
    let mut rgb = Vec::new();
    for y in 0..16u8 { for x in 0..16u8 { rgb.extend_from_slice(&[x.saturating_mul(12), y.saturating_mul(12), x.wrapping_add(y).saturating_mul(6)]); } }
    fs::write(&source, ppm(&rgb, 16, 16)).unwrap();
    let encoded = ffmpeg().arg("-hide_banner").arg("-y").arg("-i").arg(&source).arg(&jpeg).output().unwrap();
    ...
    let decoded = ffmpeg().arg("-hide_banner").arg("-y").arg("-i").arg(&jpeg).arg(&restored).output().unwrap();
...
fn grayscale_pgm_to_jpeg_and_back_is_visually_bounded() {
    ...
    assert_success(&ffmpeg().arg("-hide_banner").arg("-y").arg("-i").arg(&source).arg(&jpeg).output().unwrap());
    assert_success(&ffmpeg().arg("-hide_banner").arg("-y").arg("-i").arg(&jpeg).arg(&restored).output().unwrap());
...
fn jpeg_stream_copy_is_bit_exact() {
    ...
    assert_success(&ffmpeg().arg("-hide_banner").arg("-y").arg("-i").arg(&source).arg(&input).output().unwrap());
    let original = fs::read(&input).unwrap();
    assert_success(&ffmpeg().arg("-hide_banner").arg("-y").arg("-i").arg(&input).arg("-c:v").arg("copy").arg(&output).output().unwrap());
...
fn malformed_jpeg_fails_without_output() {
    ...
    let result = ffmpeg().arg("-hide_banner").arg("-y").arg("-i").arg(&input).arg(&output).output().unwrap();
